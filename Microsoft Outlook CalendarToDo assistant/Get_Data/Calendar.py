...
def get_events(start_date_time:str, end_date_time:str):
    ...
    if event_data.status_code == 200:
        for event in event_data.json()['value']:
            ...
            if not(event_list):
                event_list.append(f"Event: Title = {event['subject']}, Time = {event_start_date_time} {get_weekday(event_start_date_time)} - {event_end_date_time} {get_weekday(event_end_date_time)}, Location = {event['location']['displayName']}, ID = {event['id']}")
            else: 
                i = len(event_list)
