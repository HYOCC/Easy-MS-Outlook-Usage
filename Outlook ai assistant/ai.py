...
from get_data import run_data, create_event, delete_event, get_events, Create_event_with_recurrence, get_categories, create_categories
...
Changing Events Detail:
-Delete the intial event but memorize the original event's detail except for the time, Then create another event with the same detail except with the updated time.
...
def user_input():
    ...
    if user_content:
        ...
        model = genai.GenerativeModel("gemini-1.5-pro", safety_settings= Safety_rating, tools=[get_events, create_event, delete_event, Create_event_with_recurrence, get_categories, create_categories],tool_config=tool_config,system_instruction=default_ai_content)
